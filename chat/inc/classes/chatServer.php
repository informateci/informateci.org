<?php
	...
	class ChatServer {
		...
		function purgeExpired() {
			...
			$deftime = time() - $arr['time'];



			if( $deftime > $GLOBALS['fc_config']['msgRequestInterval']*5)
			{
				$arr['time'] = time() + 3600;
		...
		//delete old files
		//-------------------------------------------------
		function delete_old_cach_files()
		{
			$d = dir($GLOBALS['fc_config']['cachePath']);
			$dir = $GLOBALS['fc_config']['cachePath'];
			...
			while (false !== ($entry = $d->read()))
			{
				...
				//if ( !is_numeric(substr($entry,0,strpos($entry,'_'))) ) continue;

				$fdif = (time() - filemtime($fname));
				if($GLOBALS['fc_config']['msgRemoveAfter'] < $fdif)
				{
					unlink($fname);
				}
			}
...
//update rooms where users exists
			...
			//toLog("GLOBALS autoremoveAfter autoremoveAfter",$GLOBALS['fc_config']['autoremoveAfter']);
