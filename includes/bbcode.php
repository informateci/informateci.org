...
//DARKMOD
include_once(IP_ROOT_PATH . 'latexrender/class.latexrender.php');
// adjust this to match your system configuration
$latexrender_path = "/home/srv/httpd/informateci.org/www/www/latexrender";
...
class BBCode
{
	...
	*/
	function process_tag(&$item)
	{
		...
		*/
		...
		//	$html = '<img src="cgi-bin/mimetex.cgi?' . $content . '" alt="" border="0" style="vertical-align:middle;" />';
			...
			$latex_formula = html_entity_decode($content);
			$latex_url = $latex->getFormulaURL($latex_formula);
			...
			if ($latex_url != false) {
				$html = "<img src='".$latex_url."' title='".$alt_latex_formula."' alt='".$alt_latex_formula."' align='absmiddle'>";
			} else {
            			$html = "[unparseable or potentially dangerous latex formula]";
        		}
	...
	function process_text($text, $br = true, $chars = true)
	{
		...
		if($chars)
		{
			// FIX 1/11/2013 $text = htmlspecialchars($text);
