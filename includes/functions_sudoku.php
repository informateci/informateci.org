...
*
* @package Icy Phoenix
* @version $Id: functions_sudoku.php 96 2009-04-27 16:48:19Z Mighty Gorgon $
		...
		for ($i = 0; $i < 9; $i++)
		{
			...
			if (!$mode)
			{
				if ($tile[$tile_key] == 'x' || ($tile[$tile_key] > 9 && $tile[$tile_key] < 20))
				{
					...
					$tile_object = '<a href="' . $tile_url . '" onclick="' . $on_click . '; return false;"><img src="' . IP_ROOT_PATH . $images[$tile_image] . '" alt="' . $tile_text . '" title="' . $tile_text . '" /></a>';
				}
				else
				{
