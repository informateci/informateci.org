...
*
* @package Icy Phoenix
* @version $Id: functions_users_delete.php 96 2009-04-27 16:48:19Z Mighty Gorgon $
...
function user_clear_notifications($user_id)
{
	...
	if (!$db->sql_query($sql))
	{
		message_die(GENERAL_ERROR, 'Could not reset notifications in users table', '', __LINE__, __FILE__, $sql);
	}
	...
	if (!$db->sql_query($sql))
	{
		message_die(GENERAL_ERROR, 'Could not delete user from album comments watch table', '', __LINE__, __FILE__, $sql);
	}
