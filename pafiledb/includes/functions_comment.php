...
function display_comments(&$file_data)
{
	...
	global $_REQUEST, $phpbb_root_path, $userdata, $db, $pafiledb, $pafiledb_functions, $bbcode, $comment_bbcode_uid;
	...
	while ($comments_row = $db->sql_fetchrow($result))
	{
		...
		$poster = ( $comments_row['user_id'] == ANONYMOUS ) ? $lang['Guest'] : color_group_colorize_name($comments_row['user_id']);
		...
		if ( $comments_row['user_avatar_type'] && $poster_id != ANONYMOUS && $comments_row['user_allowavatar'] )
		{
			switch( $comments_row['user_avatar_type'] )
			{
				...
				case USER_AVATAR_GALLERY:
					...
					break;
				case USER_GRAVATAR:
					$poster_avatar = ( $board_config['enable_gravatars'] ) ? '<img src="' . get_gravatar($comments_row['user_avatar']) . '" alt="" />' : '';
					break;
			}
		}
