	...
	function get_sub_cat($cat_id)
	{
		...
		if(!empty($this->subcat_rowset[$cat_id]))
		{
			foreach($this->subcat_rowset[$cat_id] as $cat_id => $cat_row)
			{
				...
				{
					$cat_sub .= '<img src="' . $mini_img .' "/>&nbsp;<a href="' . append_sid('dload.php?action=category&cat_id=' . $cat_row['cat_id']) . '" class="forumlink2">' . $cat_row['cat_name'] . '</a> ';
				}
				else
				{
					if(!empty($this->subcat_rowset[$cat_row['cat_id']]))
					{
						foreach($this->subcat_rowset[$cat_row['cat_id']] as $sub_cat_id => $sub_cat_row)
						{
							...
							{
								$cat_sub .= '<img src="' . $mini_img .' "/>&nbsp;<a href="' . append_sid('dload.php?action=category&cat_id=' . $sub_cat_row['cat_id']) . '" class="forumlink2">' . $sub_cat_row['cat_name'] . '</a> ';
							}
						}
					}
				}
...
// Mx Addon
			...
			foreach($this->subcat_rowset[$cat_id] as $subcat_id => $subcat_row)
			{
				if( ($subcat_row['cat_allow_file'] == PA_CAT_ALLOW_FILE) )
				{
					...
					if(!empty($last_file_info['file_id']) && $this->auth[$subcat_id]['auth_read'])
					{
						...
						$last_file .= '<a href="' . append_sid('dload.php?action=file&file_id=' . $last_file_info['file_id']) . '"><img src="' . $images['icon_latest_reply'] . '" alt="' . $lang['View_latest_file'] . '" title="' . $lang['View_latest_file'] . '" /></a>';
					}
					else
					{
					...
					$pafiledb_template->assign_block_vars('no_cat_parent', array(
						...
						'CAT_IMAGE' => ($is_new) ? $images['forum_new'] : $images['forum'],
						'XS_NEW' => $xs_new,
	...
	function display_files($sort_method, $sort_order, $start, $show_file_message, $cat_id = false)
	{
		...
		for ($i = 0; $i < count($file_rowset); $i++)
		{
			...
			{
				$posticon = $phpbb_root_path . $images['link'];
			}
			//echo $posticon;
			//===================================================
			// Assign Vars
