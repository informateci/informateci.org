...
class pafiledb_toplist extends pafiledb_public
{
	...
	{
		global $pafiledb_template, $lang, $board_config, $phpEx, $pafiledb_config, $db, $images, $_REQUEST, $phpbb_root_path, $userdata;
				...
				else
				{
					...
					for ($i = 0; $i < count($file_rowset); $i++)
					{
						...
						if ($file_rowset[$i]['file_pin'] != FILE_PINNED)
						{
							...
							{
								$posticon = '<img src="' . ICONS_DIR . $file_rowset[$i]['file_posticon'] . '" />';
							}
						}
						else
						{
							$posticon = '<img src="' . $images['link'] . '" width="25" height="25" />';
						}

						$poster = (  $file_rowset[$i]['user_id'] == ANONYMOUS ) ? $lang['Guest'] : color_group_colorize_name( $file_rowset[$i]['user_id']);
						//===================================================
						// Assign Vars
						//===================================================
			...
			case 'rating':
				...
				for($i = 0; $i < count($searchset); $i++)
				{
					...
					if ($searchset[$i]['file_pin'] != FILE_PINNED)
					{
						...
						{
							$posticon = '<img src="' . ICONS_DIR . $searchset[$i]['file_posticon'] . '" />';
						}
					}
					else
					{
						$posticon = '<img src="' . $images['folder_sticky'] . '" />';
					}

					$poster = (  $searchset[$i]['user_id'] == ANONYMOUS ) ? $lang['Guest'] : color_group_colorize_name( $searchset[$i]['user_id']);
					$pafiledb_template->assign_block_vars('files_row', array(
