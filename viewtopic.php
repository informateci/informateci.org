...
if ($download)
{
	...
	$filename = ip_stripslashes($board_config['sitename']) . '_' . (ereg_replace("[^A-Za-z0-9]", "_", $post_subject)) . '_' . $disp_folder . '_' . gmdate('Ymd') . '.txt';
...
if (!$board_config['disable_ftr'])
{
	...
	if ($active && ($check_viewed == 'false') && !$bypass)
	{
		if ($viewed_mode == 'read_this')
		{
			...
			redirect(append_sid(VIEWTOPIC_MG . '?' . POST_TOPIC_URL . '=' . $ftr_topic . $kb_mode_append_red . '&mode=reading'));
		}
		else
		{
...
//<!-- END Unread Post Information to Database Mod -->
	...
	//EXSITE INFOSTUD = 19, EXSITE INFORMTECI = 69
	if( ( $poster_id == ANONYMOUS ) && ( $postrow[$i]['forum_id'] = "69" ) && ( $postrow[$i]['ext_site_id'] > "0" ) )
	{
		
		$forumfusion['1'] = "http://www.informateci.org/";
		$forumfusion['2'] = "http://www.infostud.it/";

		$avatarfusion['1'] = "member.gif";
		$avatarfusion['2'] = "member.png";
		
		$boardfusion['1'] = "informateci.org";
		$boardfusion['2'] = "InfoStud.iT";
		
	
	// RECUPERO AVATAR //
	$poster_avatar = "<img src=".$forumfusion[ $postrow[$i]['ext_site_id'] ]."images/avatars/default_avatars/".$avatarfusion[ $postrow[$i]['ext_site_id'] ]." alt='avatar'>";
	
	// RECUPERO RANK //
	$poster_qq = $postrow[$i]['post_username'];
	$user_rank_01 = " di ".$boardfusion[ $postrow[$i]['ext_site_id'] ];
	
	// RECUPERO USERNAME //
	$poster = "<a href='".$forumfusion[ $postrow[$i]['ext_site_id'] ]."profile.php?mode=viewprofile&u=".$postrow[$i]['ext_poster_id']."' target='_blank'>".$postrow[$i]['post_username']."</a>";
